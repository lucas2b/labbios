...
public class SolicitacaoDAO extends DatabaseUtil{
	...
	public boolean adicionarSolicitacao(Solicitacao solicitacao) throws ClassNotFoundException, SQLException
	{
		...
		ps.setInt(7, solicitacao.getPACIENTE_TEMPORARIO().getPACIENTE_TEMPORARIO_ID());
	...
	public boolean editarSolicitacao(Solicitacao solicitacao) throws SQLException, ClassNotFoundException
	{
		boolean retorno = false;
	...
	public List<Solicitacao> listarSolicitacoes() throws ClassNotFoundException, SQLException
	{
	...
	{
		PreparedStatement ps = getPreparedStatement("Select * from SOLICITACAO where SOLICITACAO_ID=?");
		ps.setInt(1, solicitacaoID);
		ResultSet rs = ps.executeQuery();
		return popularSolicitacao(rs);
	}

	public Solicitacao popularSolicitacao(ResultSet rs) throws ClassNotFoundException, SQLException {
		Solicitacao solicitacao = new Solicitacao();
		solicitacao.setPACIENTE(pacienteDAO.procurarPacientePorID(rs.getInt("PACIENTE_ID")));
		solicitacao.setSOL_DT_CRIACAO(rs.getDate("SOL_DT_CRIACAO"));
		solicitacao.setSTATUS(statusDAO.procurarStatusPorID(rs.getInt("STATUS_ID")));
		solicitacao.setMEDICO(medicoDAO.procurarMedicoPorID(rs.getInt("MEDICO_ID")));
		solicitacao.setANIMAL(animalDAO.procurarAnimalPorID(rs.getInt("ANIMAL_ID")));
		solicitacao.setSOL_VALOR(rs.getDouble("SOL_VALOR_DOUBLE"));
		solicitacao.setPACIENTE_TEMPORARIO(pacienteTemporarioDAO.procurarPacienteTemporarioPorID(rs.getInt("PACIENTE_TEMPORARIO_ID")));
		solicitacao.setSOL_OBS(rs.getString("PACIENTE_OBS"));
